...
First we take the log of our independent variable
...
# ggplot(training_test_data, aes(x=log_income)) + 
#   geom_histogram()
...
rf_tuning_grid3 = expand.grid(mtry = 30, splitrule = "variance", min.node.size = 100)
...
rf_caret3 = caret::train(data = training_test_data_imputed, income ~ ., method = "ranger", trControl = control5, tuneGrid = rf_tuning_grid3, importance = "impurity")
...
```{r}
#importance
rf_caret1_importance = data.frame(importance(rf_caret1_final)[order(importance(rf_caret1_final), decreasing=TRUE)])

# adjust the row and column names of the data set
names(rf_caret1_importance) = "importance"
rf_caret1_importance$var_name = rownames(rf_caret1_importance)

# select 10 most important covariates
rf_caret1_importance = rf_caret1_importance[1:10,]

# plot variable importance scores
ggplot(rf_caret1_importance, aes(x = reorder(var_name, importance, mean), y = importance)) +
  geom_point() +
...
#pred_rf_caret1_final = predict(rf_caret1_final, data = holdout_data_imputed) # Random Forest tuned with caret
